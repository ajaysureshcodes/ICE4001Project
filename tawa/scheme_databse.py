...
def setupScheme():
    ...
    schemeCrusor.execute('create index if not exists pattern_SYMBOLS on symbols (pattern asc)')
    schemeCrusor.execute('create index if not exists pattern_WORDS on words (pattern asc)')
    schemeCrusor.execute('create index if not exists wordId_WORDLIST on wordList(wordId asc)')
    schemeCrusor.execute('create index if not exists value1_SYMBOLS on symbols (value1 asc)')
    schemefile.commit()
    return True
